...
export async function createTheme({theme_name, slug, color, theme_image}) {
  const query = `
  INSERT INTO themes (theme_name, slug, color, image)
  VALUES ($1, $2, $3, $4)
  RETURNING *`;
const values = [theme_name, slug, color, image || null];
...
export async function updateTheme(id, { theme_name, slug, color, image }) {
    const result = await client.query(
        `UPDATE themes
         ...
         WHERE id = $5
         RETURNING *`,
        [theme_name, slug, color, image || null, id]
    );
